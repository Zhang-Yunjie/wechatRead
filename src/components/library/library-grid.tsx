...
export function LibraryGrid({ books }: { books: LibraryBook[] }) {
  ...
  async function setRole(bookId: string, role: "main" | "side") {
    const response = await fetch(`/api/books/${bookId}/role`, { method: "PATCH", headers: { "content-type": "application/json" }, body: JSON.stringify({ role }) });
      ...
      <div className="library-grid">{filtered.map((book) => (
        <article className="library-card" key={book.id}>
          ...
          <div className="min-w-0 flex-1"><p className="eyebrow">{book.category || "未分类"}</p><Link href={`/books/${book.id}`}><h2 className="mt-2 truncate font-serif text-xl">{book.title}</h2></Link><p className="mt-1 text-xs text-[var(--ink-soft)]">{book.author}</p><div className="mt-4"><BookProfileChips /></div>
            <div className="mt-5 flex flex-wrap gap-2"><button className="tiny-button tiny-button--strong" onClick={() => void setRole(book.id, "main")}>设为主线</button><button className="tiny-button" onClick={() => void setRole(book.id, "side")}>作为支线</button><button className="tiny-button" onClick={() => void add(book.id, "main")}>主线候选</button><button className="tiny-button" onClick={() => void add(book.id, "side")}>支线候选</button><button className="tiny-button" onClick={() => void add(book.id, "quick")}>快速阅读</button></div>
          </div>
        </article>
      ))}</div>
    </>
